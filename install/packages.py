...
def install(installType):
	...
	if installType=='1':
		#Full Install
		if cpuArch.startswith('x86'):
			os.popen('sudo apt-get -y install curl cmake g++ flex bison libgeoip-dev libpcap-dev libssl1.0-dev python-dev python-pip python-flask python-scapy apache2 libapache2-mod-wsgi swig nmap tcpdump 2>&1').read()
		else:
			os.popen('sudo apt-get -y install cmake flex bison libgeoip-dev libpcap-dev libssl1.0-dev python-dev python-pip python-flask python-scapy apache2 libapache2-mod-wsgi swig ant zip git nmap tcpdump 2>&1').read()
		...
		#Sensor Install
		if cpuArch.startswith('x86'):
			os.popen('sudo apt-get -y install curl cmake g++ flex bison libgeoip-dev libpcap-dev libssl1.0-dev python-dev python-pip python-scapy swig nmap tcpdump 2>&1').read()
		else:
			os.popen('sudo apt-get -y install cmake flex bison libpcap-dev libgeoip-dev libssl1.0-dev python-dev python-pip python-scapy swig oracle-java8-jdk ant zip git nmap tcpdump texinfo 2>&1').read()
		os.popen('sudo pip install requests 2>&1').read()
	elif installType=='3':
		#Web Server Install
		if cpuArch.startswith('x86'):
			os.popen('sudo apt-get -y install curl libgeoip-dev python-dev python-pip python-flask python-scapy apache2 libapache2-mod-wsgi 2>&1').read()
		else:
